...
class projectController extends baseController {
  ...
  async copy(ctx) {
    try {
      ...
      if (result._id) {
        ...
        for (let i = 0; i < cat.length; i++) {
          ...
          for (let key = 0; key < interfaceData.length; key++) {
            ...
            mapData.oldId = data._id;
            delete data._id;

            const interfaceRes = await this.interfaceModel.save(data);
            mapData.newId = interfaceRes._id;
            interfaceMap.set(mapData.oldId, mapData.newId);
          }
        }
      }
      ...
      yapi.emitHook('project_copy', copyData).then();
      ctx.body = yapi.commons.resReturn(result);
    } catch (err) {
      ctx.body = yapi.commons.resReturn(null, 402, err.message);
    }
